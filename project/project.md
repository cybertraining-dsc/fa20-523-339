...
In this final project, An User-Based Collaborative Filtering will be implemented to get a taste on the recommendation strategy based on Customer's Gift card purchase record and the item they also viewed and bought. 

## 2.Background

Recommendation Strategy is a quite popular research area in recent years with a strong real-world influence. Therefore, It is obvious that there are plenty of recommendation strategy have been done. The most poplular techique for recommendation systems is called Collaborative Filtering, it is a techique that can filter out items that a user might like on the basis of reactions by similar users.During which, the memory-based method is considered in this report since it used dataset to calculate the prediction by using statistical techniques and that demands the requirement for the dataset. There are two kinds of memory-based method available: *User-BasedCollaborative Filtering*, *Item-Based Collaborative Filtering*. Only User-BasedCollaborative is considered and implemented in the report since Item-Based Collaborative Filtering requires a customer review rate for evalution. This attribute is unavailable in the given dataset. [^2] 
